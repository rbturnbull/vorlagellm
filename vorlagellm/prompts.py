...
def build_source_prompt(**kwargs):    
    messages = [
        ...
        ("ai", "The {apparatus_language} readings which could be translated into the {doc_language} '{doc_corresponding_text}' are: "),
    ]
    return ChatPromptTemplate.from_messages(messages=messages).partial(**kwargs)
...
def build_corresponding_text_prompt(**kwargs):    
    messages = [
        ...
        ("user", 
            ...
            "If the section of text is missing in {doc_language}, then you should answer with 5 hyphens with no text before them. "
